...
/**
  * @brief  This function read NDEF file if NDEF is identified as TexToken
	* @param	pText : pointer on NDEF_Text_info_t structure to fill with read data
  * @retval SUCCESS : WifiToken structure has been updated
	* @retval ERROR : Not able to fill WifiToken structure
  */
uint16_t TT4_ReadTextToken(NDEF_Text_info_t *pText )
{
  uint16_t status = ERROR;
  sCCFileInfo *pCCFile;
  struct  sRecordInfo *pRecordStruct;
